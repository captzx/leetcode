...
class Solution {
public:
    uint32_t pow(int n, int e){
            if(e == 0) return 1;
            if(e == 1) return n;

            if(e % 2 == 0) return pow(n*n, e/2);
            else return n * pow(n*n, e/2);
        }
    ...
    // √ Your memory usage beats 12.5 % of cpp submissions (8.3 MB)
};
